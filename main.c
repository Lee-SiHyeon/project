...
//		 Uart_Printf("Task1 start, task current state =%d\n", current_tcb->state);
		...
		LED_0_Only_Off();
		for(i=0;i<0x100000;i++);
		if(current_tcb->state !=TASK_STATE_BLOCKED) OS_Set_Task_Block(current_tcb, 500);
	}
}
...
//		 Uart_Printf("Task2 start, task current state =%d\n", current_tcb->state);
		...
		LED_1_Only_Off();
		for(i=0;i<0x100000;i++);
		if(current_tcb->state !=TASK_STATE_BLOCKED) OS_Set_Task_Block(current_tcb, 300);
	}
...
//		 Uart_Printf("Task3 start, task current state =%d\n", current_tcb->state);
		...
		for(i=0;i<0x100000;i++);
		if(current_tcb->state !=TASK_STATE_BLOCKED) OS_Set_Task_Block(current_tcb, 100);
	}
}
...
void Main(void)
{
	...
	OS_Init();	// OS �ڷᱸ�� �ʱ�ȭ
	OS_Create_Task_Simple(Task0, (void*)0, 6, 128);
